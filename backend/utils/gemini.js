...
// Retry wrapper for transient API failures (503, timeouts, network errors)
const isTransientError = (error) => {
    const msg = error?.message || '';
    const code = error?.code || error?.status;
    return (
        code === 503 ||  // Service unavailable
        code === 500 ||  // Server error (sometimes transient)
...
const retryWithBackoff = async (fn, maxAttempts = 3, label = 'API call') => {
    let lastError;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            console.warn(`${label} failed (attempt ${attempt}/${maxAttempts}), retrying in ${delayMs}ms...`, error?.message || error);
...
export const generateMonthlyInsight = async ({
    ...
    const trendText = previousMonths.length > 0
        ? previousMonths.map(m => {
            ...
            return `- ${m.month}: Income ${currency} ${income.toFixed(2)}, Expenses ${currency} ${expenses.toFixed(2)}`;
        }).join('\n')
...
- Reference actual numbers from the data. Tone: friendly but honest.`;
    ...
    try {
        ...
        const errMessage = error?.message
            ? `Failed to generate monthly insight: ${error.message}`
...
export const analyzeTransactionList = async ({ transactions, currency = 'USD' }) => {
    ...
    const prompt = `Analyze these ${transactions.length} transactions and provide a concise, helpful spending insight. Focus on trends, anomalies, or frequent expenses.
...
export const analyzeBudgetList = async ({ budgets, currency = 'USD' }) => {
    ...
    const prompt = `You're a personal finance assistant. Analyze each budget below and provide a one-sentence status assessment.
